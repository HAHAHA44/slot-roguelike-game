...
#
# 阶段边界是这个游戏最重的一次状态变更，规则只有一条：
#   **阶段结束时该阶段的碎片全部清空，唯有满星者按 legacy_into 进化成传承物带走。**
# 传承物本身不属于任何阶段（stage_id 为空、is_legacy 为真），所以它不会被下一次清空带走，
# 会一路陪到死。见 ADR-0003。
#
...
# 阶段结束：满星者进化成传承物，其余当期碎片清空，传承物与跨阶段碎片留下。
...
func end_stage(session, tokens: Dictionary) -> Array:
	if session == null:
		...
		return []
	var kept: Array = []
	var ascending: Dictionary = {}   # domain -> 该领域触发传承的那张当期碎片定义
	for card in session.all_cards():
		...
		if card.is_max_star() and def.can_ascend() and not ascending.has(String(def.domain)):
			ascending[String(def.domain)] = def

	var ascensions: Array = []
	for domain in ascending:
		var source = ascending[domain]
